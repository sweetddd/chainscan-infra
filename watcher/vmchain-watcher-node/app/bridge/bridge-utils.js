...
const erc20Aprrove = async function(contractAddress, address, amount, decimals = 18, wallet, gasPrice = 2000000000, gasLimit = 2100000) {
  const dec = expandDecimals(amount, decimals);
  const contract = new ethers.Contract(contractAddress, GlobalConstants.GlobalConstants.ContractABIs.Erc20.abi, wallet);
  const tx = await contract.approve(address, dec, {
...
const erc20_despoit = async function(contractAddress, token, amount, dest, resourceId, data, wallet, decimals) {
  ...
  const dec = expandDecimals(amount, decimals);
  const contract = new ethers.Contract(contractAddress, GlobalConstants.GlobalConstants.ContractABIs.Bridge.abi, wallet);

  const res = await contract.deposit(dest, resourceId, data, { gasPrice: '10000000015', gasLimit: '600000' });
