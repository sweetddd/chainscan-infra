...
class EthMessageSigner {
    ...
    getOrderEthSignMessage(order) {
        ...
        if (!order.type) {
            ...
            let sell = order.direction == 'buy' ? order.tokenSell : order.tokenBuy
            let buy = order.direction == 'buy' ? order.tokenBuy : order.tokenSell
            let gasUnit = order.direction == 'buy' ? buy : sell
            let tolerance = order.type == 'limit_price' ? '' : order.tolerance[0] / order.tolerance[1] * 100 + '%'
            message +=
