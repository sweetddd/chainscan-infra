...
class BatchBuilder {
  ...
  build(feeToken) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const signature = yield this.wallet.getEthMessageSignature(message);
      return {
        txs,
        signature,
        totalFee,
      ...
      for (const tx of this.txs) {
        ...
        switch (tx.type) {
          case 'Withdraw':
            ...
            break;
          case 'Transfer':
            messages.push(yield this.wallet.getTransferEthMessagePart(tx.tx));
          ...
          case 'MintNFT':
            ...
            const mintNft = { tx: yield this.wallet.getMintNFT(tx.tx) };
            processedTxs.push(mintNft);
            break;
          case 'Swap':
            messages.push(this.wallet.getSwapEthSignMessagePart(tx.tx));
            const swap = {
              tx: yield this.wallet.getSwap(tx.tx),
              ethereumSignature: [
                null,
          ...
          case 'WithdrawNFT':
            ...
            const withdrawNft = { tx: yield this.wallet.getWithdrawNFT(tx.tx) };
